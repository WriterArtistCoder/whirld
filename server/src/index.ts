...
const DEFAULT_LANGUAGE = 'en' // Final language is English if none specified
...
app.post("/api/scramble", async (c) => {
    ...
    // TODO Make API more standard in its error handling
    ...
    try {
        ...
        for (var i = 0; i < times+1; i++) {
            if (i>0) {
                let nextLang = i==times ? LANG_INVALID?DEFAULT_LANGUAGE:lang : LANGUAGES[Math.floor(Math.random() * (LANGUAGES.length))]
                langLog += ' > ' + nextLang
                ...
                try {
                    ...
                    if (++problemsInARow < OH_CRAP_LIMIT) {
                        ...
                        try {
                            console.log(`\x1b[1;103m${new Date().toISOString()}\x1b[0m \x1b[1;101m Too many errors in a row, cutting our losses \x1b[0m \n`)
